...
const App = () => {
  ...
  const getData = (pageNo) => {
    axios
      .get(
        `${URL}/list?pageno=${pageNo}&itemsPerPage=${data.limit}`
      )
      .then((res) => {
        setData((prev) => ({
          ...prev,
  ...
  const searchdata = (searchData) => {
    if (isNaN(searchData)) {
      let searchArray = data.dataArray.filter((item) =>
        item.whose_created.includes(searchData)
      );
      setData((prev) => ({
        ...prev,
      ...
      let searchArray = data.dataArray.filter(
        (item) => item.id == parseInt(searchData)
      );
      setData((prev) => ({
        ...prev,
        dataArray: searchArray,
        ...
        <div>
          <Card bg="danger" className="my-2 text-white">
            <Card.Body>
              <Card.Text>
                Notification :{" "}
                {notification.length == 0 ? "Empty" : notification}
              </Card.Text>
            </Card.Body>
          </Card>
        </div>
      ...
      <div className="row col text-center">
        <div className="row my-2">
          ...
          <div className="col-4">
            ...
            <Button className="me-2" onClick={() => onFilter("all")}>
              All
            </Button>
            <Button className="me-2" onClick={() => onFilter("start")}>
              Start
            </Button>
            <Button className="me-2" onClick={() => onFilter("pending")}>
              Pending
            </Button>
            <Button className="me-2" onClick={() => onFilter("complete")}>
              Fulfilled
            </Button>
          </div>
          <div className="col-4">
            <h5 className="text-start">
              <b>Search by Client Name or Id</b>
            </h5>
            <div className="col-4">
